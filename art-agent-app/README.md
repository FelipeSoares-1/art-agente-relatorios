...
- 🧹 **Limpa o Conteúdo**: Converte resumos em HTML para texto puro, garantindo uma exibição limpa e legível.
- 📊 **Exibe um dashboard** com filtros por período (24h, 7d, 15d), tags, fontes e busca por termo.
...
- **O quê**: Um cron job (`cron-job.ts`) executa um "worker" a cada hora, chamando a API `/api/enrich-articles`.
...
│   │   └── enrich-articles/  # GET /api/enrich-articles - Endpoint do Worker de enriquecimento
│   ├── dashboard/            # Página do dashboard principal
│   └── page.tsx              # Página inicial (que renderiza o dashboard)
├── lib/
